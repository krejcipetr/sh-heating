...
function cometblue_receiveconf ( $a_mac, $a_pin ) {
	$l_output = cometblue_commreceive_gatt ( $a_mac, $a_pin );
	if ( $l_output === false ) {
		return false;
	}
...
function cometblue_commreceive_gatt ( $a_mac, $a_pin ) {
	$l_output = array ();
...
function cometblue_sendconf ( $a_radiator, $a_pin ) {
	...
	if ( false === cometblue_commsend_gatt ( $a_radiator ['mac'], $a_pin, $l_output ) ) {
		return false;
	}
...
/**
 * Poslani pres except a gatt-clienta do hlavice
 *
 * @param string $a_pin        	
 * @param string $a_mac        	
 * @param string[] $a_data        	
 ...
 * @return boolean
 */
function cometblue_commsend_gatt ( $a_mac, $a_pin, $a_data ) {
	/* Nahrání do zažízení */
	try {
		...
		foreach ( array ('0x001b', '0x001d', '0x001f', '0x0021', '0x0023', '0x0025', '0x0027', '0x0029', '0x002b', '0x002d', '0x002f', '0x0031', '0x0033', '0x0035', '0x0037', '0x0039', '0x003b', '0x003d' ) as $l_idx => $l_handle ) {
			fwrite ( $stream, "write-value " . $l_handle . " " . $a_data [$l_idx] . "\n" );
