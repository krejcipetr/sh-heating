...
function dovolena_decode ( $l_casti ) {
	...
	return array(
		'from' => sprintf ( "%02d/%02d/%04d %02d", hexdec ( $l_casti [ 2 ] ), hexdec ( $l_casti [ 1 ] ), hexdec ( $l_casti [ 3 ] ) + 2000, hexdec ( $l_casti [ 0 ] ) % 24 ),
		'to'   => sprintf ( "%02d/%02d/%04d %02d", hexdec ( $l_casti [ 6 ] ), hexdec ( $l_casti [ 5 ] ), hexdec ( $l_casti [ 7 ] ) + 2000, hexdec ( $l_casti [ 4 ] ) % 24 ),
...
function cometblue_open ( $a_mac, $a_pin, $a_max_retries = 3 ) {
	/* Nacteni ze zažízení */
...
function cometblue_receiveconf ( $a_mac, $a_pin ) {
	try {
		...
		foreach ( $l_readdefs as $l_readdef ) {
			fwrite ( $stream, 'read-value ' . $l_btdefs[ $l_readdef[ 0 ] ] . "\n" );

			$l_data = array();
			switch ( expect_expectl ( $stream, $cases, $l_data ) ) {
				case "data" :
					// Rozpadni data podle nazctenych udaju
					$l_output [ $l_readdef[ 0 ] ] = explode ( " ", $l_data [ 1 ] );
					break;
...
function cometblue_sendconf ( $a_radiator, $a_pin ) {
	...
	try {
		...
		list( $stream, $l_btdefs ) = cometblue_open ( $a_radiator['mac'], $a_pin );
		...
		foreach ( array(16 => $l_pins [ 16 ], 17 => $l_pins [ 17 ]) as $l_idx => $l_handle ) {
			...
			switch ( expect_expectl ( $stream, $cases, $l_match ) ) {
				case "data" :
					$l_source [ $l_idx ] = explode ( " ", $l_match [ 1 ] );
					break;
				case 'nodata' :
					break;
				default :
		...
		$l_output [] = sprintf ( "0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s 0x%02s", UNCHANGED_VALUE, dechex ( $a_radiator [ 'required' ] * 2 ), dechex ( $a_radiator [ 'night' ] * 2 ), dechex ( $a_radiator [ 'comfort' ] * 2 ),
