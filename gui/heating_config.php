...
if (isset($_REQUEST['dovolena'])) {
    ...
    foreach ($GLOBALS['heating']['radiators'] as $l_idx => $l_radiator) {
        $GLOBALS['heating']['radiators'][$l_idx]['dovolena'] = array(
                'from' => strftime("%x %X", $l_from),
                'to' => strftime("%x %X", $l_to),
                "temp" => floatval($_REQUEST["temp"])
        );
        ...
        foreach (array(
            ...
            for ($l_idx2 = 0; $l_idx2 < 4; $l_idx2 ++) {
                if (! preg_match("/[0-9]{1,2}:[0-9]{1,2}/",
                        $_REQUEST['from' . $l_den . $l_idx2]) ||
                         ! preg_match("/[0-9]{1,2}:[0-9]{1,2}/",
                                $_REQUEST['to' . $l_den . $l_idx2])) {
                    if ($_SESSION['radiator'] != - 1) {
                        unset($l_radiator[$l_den][$l_idx2]);
                    }
                    continue;
                }
                $l_radiator[$l_den][$l_idx2] = array(
                        'from' => $_REQUEST['from' . $l_den . $l_idx2],
                        'to' => $_REQUEST['to' . $l_den . $l_idx2]
                );
            }
        }
...
switch ($_SESSION['mode']) {
    ...
    case 0:
        ...
        foreach ($GLOBALS['heating']['radiators'] as $l_idx => $l_radiator) {
            echo '<input type="button" value="', htmlspecialchars(
                    $l_radiator['name']), '" onclick="document.location.href=\'heating_config.php?radiator=', $l_idx, '\';">';
        }
        ...
        if ($_SESSION['radiator'] != - 1) {
            ...
            echo '<div style="text-align:center;"><h1>', htmlspecialchars(
                    $l_radiator['name']), '</h1></div>';
        } else {
            $l_radiator = array();
        }
...
<?php
        foreach (array(
            ...
            echo '<div class="panel programator_den"><div class="label">';
            echo htmlspecialchars($l_den);
            echo "</div>";
            $l_idx2 = null;
