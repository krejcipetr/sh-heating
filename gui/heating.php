...
<meta http-equiv="refresh"
	content="<?php echo strtotime($GLOBALS['heating']['next']) -time() + 10;?>; url=heating.php">
...
foreach ($GLOBALS['heating']['sources'] as $l_idx => $l_source) {
    ?>
		...
		<div class="label"><?php echo htmlspecialchars($l_source['name'])?></div>
		<img class="heating"
			src="<?php echo ($l_source['state'])?'heating.png':'noheating.png';?>"
			title="topeni">
		<!-- Napajene radiatory -->
		<div class="for">
		<?php
    ...
    $l_cas = ($l_source['runningfrom'] != null) ? intval(
            (time() - strtotime($l_source['runningfrom'])) / 60) : null;
    ?>
    <!-- Delka behu zdroje -->
		<div class="runningtime"><?php echo $l_source['runningfrom'];?><br><?php if ($l_cas ) { echo $l_cas,"&nbsp;min"; }?><br>
			<br><?php printf ("%.1f&nbsp;hod", floatval(($l_source['summary'] + $l_cas)/60));?></div>
	</div>
<?php
}
// RADIATORY
foreach ($GLOBALS['heating']['radiators'] as $l_idx => $l_radiator) {
    ?>    
		...
		<span class="aktualni"><?php printf("%.1f",$l_radiator["current"]);?></span> <span
			class="required"><?php printf("%s", $l_radiator["required"]);?> °C</span> <img
			class="heating"
