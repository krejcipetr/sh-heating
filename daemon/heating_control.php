...
foreach (array_keys($GLOBALS['heating']['radiators']) as $l_idx) {
    ...
    $l_radiator['control']['heating'] = $g_heating;
    $l_radiator['control']['curr_from'] = time();
    $l_radiator['control']['direction'] = $l_radiator_now['required'] >=
             $l_radiator['comfort'];
}
printf("End" . PHP_EOL);
...
while (true) {
    ...
    foreach ($l_radiators as $l_idx) {
        ...
        if ($l_radiator['required'] != $l_radiator_now['required']) {
            ...
            if ($l_radiator['control']['direction']) {
                $l_heating = true;
                $l_radiator['control']['state'] = sprintf(
                        'heating-requp-zmena teploty na vyssi %s->%s',
        ...
        } elseif ($l_radiator['control']['heating'] &&
                 $l_radiator_now['current'] < $l_radiator['current']) {
            $l_heating = false;
            $l_radiator['control']['state'] = 'cooling-on-tempdown-hlavice se zavrela';
        } elseif ($l_radiator['control']['heating'] &&
                 (time() - $l_radiator['control']['curr_from']) > HEATING_TIMEOUT) {
            $l_heating = false;
        ...
        } elseif (! $l_radiator['control']['heating'] && ! $l_heating_curr &&
                 $l_radiator['control']['direction'] &&
                 $l_radiator_now['current'] < $l_radiator['current']) {
            $l_heating = true;
        ...
        if ($l_radiator_now['battery'] < BATTERY_LIMIT) {
            $l_lowbattery[] = $l_radiator['mac'] . "-" . $l_radiator['name'] .
                     "-" . $l_radiator_now['battery'];
        }
    }
    ...
    foreach (array_keys($GLOBALS['heating']['sources']) as $l_idx) {
        ...
        if ($l_state != $l_source['state']) {
            if ($l_state == false) {
                $l_source['summary'] += (time() -
                         strtotime($l_source['runningfrom'])) / 60;
    ...
    // Poslani emailu
    mail("root", "HEATING  Report " . implode(",", $l_message_subject),
            $l_message_subjectreport .
                     ((testing) ? var_export($GLOBALS['heating'], true) : ""));
